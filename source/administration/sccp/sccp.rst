...
Starting from Wazo 1X.XX, an SCCP device can be associated to a user by entering the user's provisioning code
...
* the :guilabel:`Allow guest connections` option must be enabled to allow SCCP devices to connect to
  ...
  provide some additional security if your XiVO is in an hostile environment, at the cost of making
...
* the :guilabel:`Maximum number of guest connections` option limits the number of SCCP devices that
  ...
  XiVO is in an hostile environment, where you should probably set it to a fairly low value.
